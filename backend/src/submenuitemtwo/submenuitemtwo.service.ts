...
export class SubmenuItemTwoService {
  ...
  async update(id: number, updateSubmenuItemInput: UpdateSubmenuItemTwoInput): Promise<SubmenuItemTwoEntity> {
    const submenu = await this.findOne(id)
    const newSubMenu = {...submenu, ...updateSubmenuItemInput, menuitem: { id: updateSubmenuItemInput.submenuitem_id }}
